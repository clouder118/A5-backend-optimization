...
const mapImagePromises = new Map<string, Promise<void>>();
...
export function preloadScenicMapImage(map: ScenicMap): Promise<void> {
  if (typeof window === 'undefined') return Promise.resolve();
  const existing = mapImagePromises.get(map.imageUrl);
  if (existing) return existing;
  ...
  mapImagePromises.set(map.imageUrl, request);
  return request;
}
