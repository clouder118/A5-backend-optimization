...
async function main() {
  ...
  try {
    ...
    await waitForPath(page, '/routes');
    await page.waitForTimeout(300);
    failIf(await page.getByTestId('route-hydra-loader').isVisible().catch(() => false), 'Route loader should not appear during visitor page navigation');
    await page.getByTestId('route-workbench').waitFor({ timeout: 8000 });

    await page.getByRole('button', { name: '开始游览' }).click();
    await waitForPath(page, '/guide');
    await page.getByRole('dialog', { name: '路线导览' }).waitFor({ timeout: 8000 });
    await page.getByText('景区入口').first().waitFor({ timeout: 5000 });
    await page.getByRole('button', { name: '关闭路线以与导游交流' }).waitFor({ timeout: 5000 });
    await page.getByRole('button', { name: '关闭路线以与导游交流' }).click();
    await page.getByRole('dialog', { name: '路线导览' }).waitFor({ state: 'hidden', timeout: 5000 });
    await page.getByRole('button', { name: '打开路线' }).click();
    await page.getByRole('dialog', { name: '路线导览' }).waitFor({ timeout: 5000 });
    await page.screenshot({ path: path.join(outputDir, '02-guide-route-drawer-open.png'), fullPage: true });

    await page.getByRole('dialog', { name: '路线导览' }).getByRole('button', { name: /下一站/ }).click();
    await page.getByText('五明桥').first().waitFor({ timeout: 5000 });
    await page.screenshot({ path: path.join(outputDir, '03-guide-route-drawer-progress.png'), fullPage: true });
    await page.getByRole('button', { name: '上一站' }).click();
    await page.getByText('景区入口').first().waitFor({ timeout: 5000 });
    await page.getByRole('button', { name: '提前结束本次游览' }).click();
    await page.getByText('本次游览已结束').first().waitFor({ timeout: 5000 });
    await page.locator('.visitor-header').getByRole('link', { name: /\[ 路线 \]/ }).click();
    await waitForPath(page, '/routes');
    failIf(page.url().includes('/tour/tour-guide-1'), 'Route navigation should return to recommendation page instead of deep tour page');
    await page.screenshot({ path: path.join(outputDir, '04-route-nav-returns-to-routes.png'), fullPage: true });
