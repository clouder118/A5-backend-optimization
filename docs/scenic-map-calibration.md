...
- 服务设施专用标注工具：`http://127.0.0.1:5173/tools/service-point-calibrator.html`
...
服务设施使用独立工具，不包含景点选择、道路描绘和时间标定：

1. 打开 `http://127.0.0.1:5173/tools/service-point-calibrator.html`。
2. 选择“灵山胜境”或“拈花湾”。
3. 选择服务类别并填写名称，然后直接点击地图上的设施位置。
4. 点击已有点位可以选中，拖动点位可以修正位置；右侧列表会显示比例坐标。
5. 使用“复制完整地图 JSON”或“下载完整地图 JSON”。导出的文件会保留景点点位，并追加当前服务点位。
6. 将对应地图的完整 JSON 放回 `knowledge/maps/ling-shan-overview-v1.json` 或 `knowledge/maps/nianhua-bay-overview-v1.json`，然后重启后端。

服务类别必须使用以下固定类型：`restroom`、`dining`、`lodging`、`shop`、`shuttle`。这些类型会被路线、推荐和普通景点列表过滤掉，只在服务地图中显示。
