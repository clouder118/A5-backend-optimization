...
$VisitorLog = Join-Path $Frontend "frontend-visitor-local-server-5173.log"
$VisitorErr = Join-Path $Frontend "frontend-visitor-local-server-5173.err.log"
$AdminLog = Join-Path $Frontend "frontend-admin-local-server-5174.log"
$AdminErr = Join-Path $Frontend "frontend-admin-local-server-5174.err.log"
...
function Start-Frontend {
  ...
  Write-Step "[6/7] Starting $Entry frontend http://127.0.0.1:$Port"
  if (Test-Path $StdoutPath) { Clear-Content -LiteralPath $StdoutPath }
  if (Test-Path $StderrPath) { Clear-Content -LiteralPath $StderrPath }
  Start-BackgroundCommand `
    -WorkingDirectory $Frontend `
    -Command "node ..\scripts\serve-frontend-static.mjs" `
...
function Start-BackgroundCommand {
  ...
  foreach ($key in $EnvironmentVariables.Keys) {
    $processInfo.Environment[$key] = $EnvironmentVariables[$key]
  }
...
try {
  ...
  Start-Backend
  Start-Frontend -Port 5173 -Entry "visitor" -StdoutPath $VisitorLog -StderrPath $VisitorErr
  Start-Frontend -Port 5174 -Entry "admin" -StdoutPath $AdminLog -StderrPath $AdminErr

  Write-Step "[7/7] Waiting for services"
  $backendReady = Wait-Url "http://127.0.0.1:8001/health" 30
  $visitorReady = Wait-Url "http://127.0.0.1:5173/" 30
  $adminReady = Wait-Url "http://127.0.0.1:5174/" 30

  Write-Host ""
  if ($backendReady -and $visitorReady -and $adminReady) {
  ...
  Write-Host "Visitor log: frontend\frontend-visitor-local-server-5173.log"
  Write-Host "Admin log: frontend\frontend-admin-local-server-5174.log"
  Write-Host "Mode: static frontend servers, no Vite hot reload."
  Write-Host "After code changes: run BUILD-FRONTEND.bat, then START-HERE.bat."

  if (-not $NoBrowser -and $visitorReady) {
