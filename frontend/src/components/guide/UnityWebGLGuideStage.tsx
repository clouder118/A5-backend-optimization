...
export default function UnityWebGLGuideStage({
  ...
  useEffect(() => {
    ...
    sendEmotionCue(emotionCue);
    sendState(resolveAvatarState(status, audioState, emotionCue));
  ...
  useEffect(() => {
    if (phase !== 'ready' || !active) return;
    if (resolveAvatarState(status, audioState, emotionCue) !== 'idle') return;
    ...
    >
      {phase !== 'fallback' ? (
        ...
        <div className="unity-avatar-static-fallback" aria-label="151 AI guide static fallback">
          <img src={fallbackImageUrl} alt="灵山胜境 AI 数字人导游" draggable={false} />
