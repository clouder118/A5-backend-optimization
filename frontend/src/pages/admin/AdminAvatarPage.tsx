...
export default function AdminAvatarPage() {
  ...
  const current = useMemo(() => items.find((item) => item.isActive), [items]);
      ...
      render: (name: string, item) => (
        <div style={{ minWidth: 0, wordBreak: 'break-word' }}>
          <Space size="small" wrap>
            ...
            {item.isBuiltin ? <Tag>内置</Tag> : null}
          </Space>
          <div><Typography.Text type="secondary">{item.note || '暂无备注'}</Typography.Text></div>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_, item) => (
        <Space direction="vertical" size={0}>
          <Typography.Text>{item.sourceFilename || (item.isBuiltin ? '项目内置' : '-')}</Typography.Text>
    ...
    <div className="admin-page">
      ...
      <Space align="start" style={{ justifyContent: 'space-between', width: '100%' }} wrap>
        <Space direction="vertical" size={4}>
          <Typography.Text className="mono-label">[ DIGITAL HUMAN ]</Typography.Text>
          <Typography.Title level={2}>数字人形象管理</Typography.Title>
          <Typography.Text type="secondary">
            管理本地 Unity WebGL 数字人版本，当前切换不会改动游客端其他功能。
          </Typography.Text>
        </Space>
          ...
          <Space align="start">
            ...
            <Space direction="vertical" size={0}>
              <Space>
                ...
                {current.isBuiltin ? <Tag>内置默认</Tag> : null}
              </Space>
