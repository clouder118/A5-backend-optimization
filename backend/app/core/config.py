...
def default_embedding_index_on_startup() -> str:
    explicit_value = os.getenv("RAG_EMBEDDING_INDEX_ON_STARTUP")
    if explicit_value is not None:
        return explicit_value
    is_pytest = os.getenv("PYTEST_CURRENT_TEST") or any(
...
class Settings:
    ...
    guide_warmup_on_startup: str = field(
        default_factory=lambda: os.getenv(
            "GUIDE_WARMUP_ON_STARTUP",
            "false" if os.getenv("PYTEST_CURRENT_TEST") or any("pytest" in Path(arg).name.lower() for arg in sys.argv) else "true",
        )
    )
