...
const visitorLog = path.join(frontend, 'frontend-visitor-local-server-5173.log');
const visitorErr = path.join(frontend, 'frontend-visitor-local-server-5173.err.log');
const adminLog = path.join(frontend, 'frontend-admin-local-server-5174.log');
const adminErr = path.join(frontend, 'frontend-admin-local-server-5174.err.log');
...
function startDetached(file, args, cwd, stdoutPath, stderrPath, env = {}) {
...
function startServices() {
  ...
  startDetached(
    ...
    visitorLog,
    visitorErr,
    { FRONTEND_PORT: '5173', FRONTEND_ENTRY: 'visitor' },
  );
  startDetached(
    process.execPath,
    [path.join(root, 'scripts', 'serve-frontend-static.mjs')],
    ...
    adminLog,
    adminErr,
    { FRONTEND_PORT: '5174', FRONTEND_ENTRY: 'admin' },
  );
}
...
async function main() {
  ...
  const visitorReady = await waitUrl('http://127.0.0.1:5173/', 30);
  const adminReady = await waitUrl('http://127.0.0.1:5174/', 30);

  if (!backendReady || !visitorReady || !adminReady) {
  ...
  console.log('');
  console.log('Visitor log: frontend\\frontend-visitor-local-server-5173.log');
  console.log('Admin log: frontend\\frontend-admin-local-server-5174.log');
  console.log('Mode: static frontend servers, no Vite, no hot reload.');
